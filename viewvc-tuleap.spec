...
%{__perl} -pi \
    ...
    -e 's|CONF_PATHNAME =.*|CONF_PATHNAME = r"%{confdir}/%{app_name}.conf"|g;' \
    $(find %{buildroot}%{_datadir}/viewvc/ -type f)

#perl -p -i -e 's|<VIEWVC_INSTALL_DIRECTORY>|%{_datadir}/%{app_name}|g;' viewvc.conf.dist
pwd
...
%attr(644, %{codendi_user}, %{codendi_group}) %config(noreplace) %{confdir}/%{app_name}.conf
...
* Wed Mar 09 2011 Manuel Vacelet <manuel.vacelet@st.com> - 1.0.7-3
- Rename package based on parent application name (PKG_NAME)

* Wed Mar 09 2011 Manuel Vacelet <manuel.vacelet@st.com> - 1.0.7-2.codendi
- Add dependency on codendi rpm
